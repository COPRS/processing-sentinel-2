...
send_to_prip() {
  touch batch.sftp
  chmod 666 "$1"
  echo "Pushing $1 to ${PRIP_INGESTION_DIR}/ folder"
...
has_already_been_in_error(){
  local LOCKS
  LOCKS=$(find "$(dirname "$1")" -maxdepth 1 -type f -name '.error_'"$(basename "$1")"'_*' | grep "$HOST" | tail -n 1)
  if [ -z "$LOCKS" ]; then
    echo "NO"
  else
    echo "YES"
...
if [ $# -lt 2 ]; then
  echo "Launch_L2.sh PRODUCTS_DIR TIME_SLEEP_LOOP"
  exit 1
fi
...
WORK_DIR=/mnt/local/L2_stub/work_$HOST
OUT_DIR=/mnt/local/L2_stub/out_$HOST
LOG_DIR=/mnt/shared/STUB_L2REPORTS/$HOST
PRIP_DIR=/mnt/local/STUB_PRIP_L2_$HOST
KPI_DIR=/mnt/local/STUB_KPI_L2_$HOST
#Local
...
while true; do
  ...
  find "${PRODUCT_DIR}" -maxdepth 3 -type f -name "S2*L1C_TL*tar" >"${WORK_DIR}/ListTL.txt"
  shuf "${WORK_DIR}/ListTL.txt" > "${WORK_DIR}/ListTLRandom.txt"

  while IFS= read -r f; do

    if [ ! -f "$f" ];then
      echo "product already remove by another instance, exit loop to refresh list"
      break
    fi
    ...
    PROD=$f
    echo "Found $PROD"
    #Find the associated DS
    PROD_DS_L2=$(find "$(dirname "$(dirname "$PROD")")/DS" -name "S2*L2A_DS*tar" | sort | head -1)
    if [ -z "${PROD_DS_L2}" ]; then
      echo "Datastrip L2 not available, skip"
      continue
    fi
    echo "Corresponding datastrip L2: $PROD_DS_L2"
    PROD_DS=$(find "$(dirname "$(dirname "$PROD")")/DS" -name "S2*L1C_DS*tar")
    if [ -z "${PROD_DS}" ]; then
      echo "Datastrip L1 not available, skip"
      continue
    fi
    echo "Corresponding datastrip L1: $PROD_DS"
    ...
    if [ "$OWN_ERROR" == "YES" ]; then
      echo "$(date +%Y%m%d%H%M) : Product $PROD is in error on this host already"
      continue
    fi
    IN_ERROR=$(has_too_much_errors "$PROD")
    if [ "$IN_ERROR" == "YES" ]; then
      echo "$(date +%Y%m%d%H%M) : Product $PROD is in error more than 3 times"
      continue
    fi
    if [ -f "$(dirname "$PROD")/.treated_$(basename "$PROD")" ]; then
      echo "$(date +%Y%m%d%H%M) : Product already treated"
      continue
    fi
    if [ -f "$(dirname "$PROD")/.lock_$(basename "$PROD")" ]; then
      echo "$(date +%Y%m%d%H%M) : Product $PROD is locked by another instance"
      continue
    fi

    has_locks=$(has_others_lock "$PROD")
    echo "has_lock: $has_locks"
    if [ "$has_locks" == "YES" ]; then
      echo "$(date +%Y%m%d%H%M) : Product $PROD is claimed by another instance"
      continue
    else
      ...
      if [ "$has_locks" == "YES" ]; then
        ...
        if [ "$is_older" == "YES" ]; then
          ...
          echo "$(date +%Y%m%d%H%M) : Product $PROD : we are not the best"
    ...
    if [ -f "$(dirname "$(dirname "$PROD")")/originDate.txt" ]; then
      echo "$(date +%Y%m%d%H%M) : $(dirname "$(dirname "$PROD")")/originDate.txt found"
      ORIGIN_DATE=$(cat "$(dirname "$(dirname "$PROD")")/originDate.txt")
    else
      echo "$(date +%Y%m%d%H%M) : OriginDate not available, skip"
      rm -f "$(dirname "$PROD")/.lock_$(basename "$PROD")_$HOST"
      rm -f "$(dirname "$PROD")/.lock_$(basename "$PROD")"
      continue
    fi
    ...
    if [ -z "${GIP_L2ACSC}" ]; then
      ...
      GIP_L2ACSC=${CUR_DIR}/L2A_GIPPs_Sen2Cor_02.10.00/L2A_CAL_SC_GIPP.xml
    fi
    ...
    if [ -z "${GIP_L2ACAC}" ]; then
      ...
      GIP_L2ACAC=${CUR_DIR}/L2A_GIPPs_Sen2Cor_02.10.00/L2A_CAL_AC_GIPP.xml
    fi
    ...
    echo "$(date +%Y%m%d%H%M) : Treating L1C product : $PROD, originDate: ${ORIGIN_DATE} , tmp: ${TEMP_WORK} , out: ${TEMP_OUT} , log: ${LOG_FILE}, err: ${ERR_FILE}"
    echo "$(date +%Y%m%d%H%M) : Treating L1C product : $PROD, originDate: ${ORIGIN_DATE} , tmp: ${TEMP_WORK} , out: ${TEMP_OUT} , log: ${LOG_FILE}, err: ${ERR_FILE}" >"${LOG_FILE}"
    echo "$(date +%Y%m%d%H%M) : Treating L1C product : $PROD, originDate: ${ORIGIN_DATE} , tmp: ${TEMP_WORK} , out: ${TEMP_OUT} , log: ${LOG_FILE}, err: ${ERR_FILE}" >"${ERR_FILE}"
    ...
    echo "$(date +%Y%m%d%H%M) : Extracting TL tar " >> "${LOG_FILE}"
    if [ -f "$PROD" ];then
      mkdir -p "${TEMP_WORK}"/TL
      tar xf "${PROD}" -C "${TEMP_WORK}"/TL/
      retVal=$?
      ...
      if [ $retVal -ne 0 ]; then
        ...
        echo "$(date +%Y%m%d%H%M) : Failed to decompress $PROD" >> "${LOG_FILE}"
        echo "$(date +%Y%m%d%H%M) : Failed to decompress $PROD" >> "${ERR_FILE}"
        rm "$(dirname "$PROD")/.lock_$(basename "$PROD")"
        rm "$(dirname "$PROD")/.lock_$(basename "$PROD")_$HOST"
        touch "$(dirname "$PROD")/.error_$(basename "$PROD")_tar_$HOST"
        echo "FAILED" >"${STATUS_FILE}"
        continue
      fi
    else
        echo "$(date +%Y%m%d%H%M) : Product no more reachable $PROD"
        echo "$(date +%Y%m%d%H%M) : Product no more reachable $PROD" >> "${LOG_FILE}"
        echo "$(date +%Y%m%d%H%M) : Product no more reachable $PROD" >> "${ERR_FILE}"
        rm "$(dirname "$PROD")/.lock_$(basename "$PROD")"
        rm "$(dirname "$PROD")/.lock_$(basename "$PROD")_$HOST"
        continue
    fi
    ...
    if [ -f "$PROD_DS" ];then
      ...
      echo "$(date +%Y%m%d%H%M) : Extracting DS tar " >> "${LOG_FILE}"
      WORK_TL=$(find_one_file "${TEMP_WORK}/TL/" "S2*L1C_TL*")
      mkdir -p "${TEMP_WORK}/DS"
      tar xf "${PROD_DS}" -C "${TEMP_WORK}/DS/"
      retVal=$?
      handle_return $retVal "$PROD" "${LOG_FILE}" "${ERR_FILE}" "${TEMP_WORK}" "${TEMP_OUT}" "$PROD"
      echo "RetVal detar "$retVal
      if [ $retVal -ne 0 ]; then
        echo "$(date +%Y%m%d%H%M) : Failed to decompress $PROD_DS"
        echo "$(date +%Y%m%d%H%M) : Failed to decompress $PROD_DS" >> "${LOG_FILE}"
        echo "$(date +%Y%m%d%H%M) : Failed to decompress $PROD_DS" >> "${ERR_FILE}"
        rm "$(dirname "$PROD")/.lock_$(basename "$PROD")"
        rm "$(dirname "$PROD")/.lock_$(basename "$PROD")_$HOST"
        touch "$(dirname "$PROD_DS")/.error_$(basename "$PROD_DS")_tar_$HOST"
        echo "FAILED" >"${STATUS_FILE}"
        continue
      fi
    else
        echo "$(date +%Y%m%d%H%M) : Product no more reachable $PROD_DS"
        echo "$(date +%Y%m%d%H%M) : Product no more reachable $PROD_DS" >> "${LOG_FILE}"
        echo "$(date +%Y%m%d%H%M) : Product no more reachable $PROD_DS" >> "${ERR_FILE}"
        rm "$(dirname "$PROD")/.lock_$(basename "$PROD")"
        rm "$(dirname "$PROD")/.lock_$(basename "$PROD")_$HOST"
        continue
    fi
    ...
    sed -i 's+\@\@\@MSI_L2A_DS\@\@\@+'"$(dirname "$PROD_DS_L2")"'+' "$JOB_ORDER"
    sed -i 's+\@\@\@MSI_L2A_TL\@\@\@+'"${TEMP_OUT}/L2_TL"'+' "$JOB_ORDER"
    sed -i 's+\@\@\@KPI_L2A\@\@\@+'"${TEMP_OUT}/L2_KPI"'+' "$JOB_ORDER"

    echo "$(date +%Y%m%d%H%M) : Start L2" >>"${LOG_FILE}"
    launch_step_python "$CUR_DIR/WrapperCore/sen2cor_launcher.py" --joborder "$JOB_ORDER" --mode TL | tee -a "${LOG_FILE}"
    retVal=$?
    handle_return $retVal "$PROD" "${LOG_FILE}" "${ERR_FILE}" "${TEMP_WORK}" "${TEMP_OUT}" "$PROD"
    echo "RetVal "$retVal
    if [ $retVal -ne 0 ]; then
    ...
    mv "${TEMP_OUT}"/L2_TL/* "${PRIP_DIR}"/

    # Gen SIZE and txt files
    find "${PRIP_DIR}/" -name "*.tar" -print0 | while read -rd $'\0' k; do

      echo "$(date +%Y%m%d%H%M) : Product : $k"
      echo "$(date +%Y%m%d%H%M) : Product : $k" >>"${LOG_FILE}"
    ...
    if [ -z ${PRIP_PASSWORD+x} ]; then
      ...
      for j in "${PRIP_DIR}"/*.tar; do
        echo "$(date +%Y%m%d%H%M) : Pushing $(dirname "$j")/$(basename "$j" | sed 's+.tar++').txt to PRIP"
        echo "$(date +%Y%m%d%H%M) : Pushing $(dirname "$j")/$(basename "$j" | sed 's+.tar++').txt to PRIP" >> "${LOG_FILE}"
        send_to_prip "$(dirname "$j")/$(basename "$j" | sed 's+.tar++').txt"
        retVal=$?
        if [ $retVal -ne 0 ]; then
          echo "$(date +%Y%m%d%H%M) : Error pushing $j txt"
          echo "$(date +%Y%m%d%H%M) : Error pushing $j txt" >>"${LOG_FILE}"
          echo "$(date +%Y%m%d%H%M) : Error pushing $j txt" >>"${ERR_FILE}"
          echo "$(date +%Y%m%d%H%M) : Pushed $j txt to PRIP : NK" >>"${PRIP_FILE}"
        else
          echo "$(date +%Y%m%d%H%M) : Pushing $j txt is OK"
          echo "$(date +%Y%m%d%H%M) : Pushing $j txt is OK" >>"${LOG_FILE}"
          echo "$(date +%Y%m%d%H%M) : Pushed $j txt to PRIP : OK" >>"${PRIP_FILE}"
          rm -f "$(dirname "$j")/$(basename "$j" | sed 's+.tar++').txt"
          echo "$(date +%Y%m%d%H%M) : Pushing $j to PRIP"
          echo "$(date +%Y%m%d%H%M) : Pushing $j to PRIP" >>"${LOG_FILE}"
          send_to_prip "$j"
          retVal=$?
          if [ $retVal -ne 0 ]; then
            echo "$(date +%Y%m%d%H%M) : Error pushing $j"
            echo "$(date +%Y%m%d%H%M) : Error pushing $j" >>"${LOG_FILE}"
            echo "$(date +%Y%m%d%H%M) : Error pushing $j" >>"${ERR_FILE}"
            echo "$(date +%Y%m%d%H%M) : Pushed $j to PRIP : NK" >>"${PRIP_FILE}"
          else
            echo "$(date +%Y%m%d%H%M) : Pushing $j is OK"
            echo "$(date +%Y%m%d%H%M) : Pushing $j is OK" >>"${LOG_FILE}"
            echo "$(date +%Y%m%d%H%M) : Pushed $j to PRIP : OK" >>"${PRIP_FILE}"
            rm -f "$j"
          fi
        fi
      done
      for j in "${PRIP_DIR}"/*.xml; do
    ...
    if [ -z ${KPI_PASSWORD+x} ]; then
      ...
      for j in "${KPI_DIR}"/*SIZE.xml; do
        echo "$(date +%Y%m%d%H%M) : Pushing $j to KPI"
        echo "$(date +%Y%m%d%H%M) : Pushing $j to KPI" >>"${LOG_FILE}"
        send_to_kpi "$j" files/INBOX/PRODUCTION/SIZE
        retVal=$?
        if [ $retVal -ne 0 ]; then
          echo "$(date +%Y%m%d%H%M) : Error pushing KPI $j !!!!"
          echo "$(date +%Y%m%d%H%M) : Error pushing KPI $j" >>"${LOG_FILE}"
          echo "$(date +%Y%m%d%H%M) : Error pushing KPI $j" >>"${ERR_FILE}"
          echo "$(date +%Y%m%d%H%M) : Pushed $j to KPI : NK" >>"${KPI_FILE}"
        else
          echo "$(date +%Y%m%d%H%M) : Pushing $j to KPI is OK"
          echo "$(date +%Y%m%d%H%M) : Pushing $j to KPI is OK" >>"${LOG_FILE}"
          echo "$(date +%Y%m%d%H%M) : Pushed $j to KPI : OK" >>"${KPI_FILE}"
          rm -f "$j"
        fi
      done
    fi
...
#      done
#    fi
    echo "$(date +%Y%m%d%H%M) : Success treating $PROD"
    echo "$(date +%Y%m%d%H%M) : Success treating $PROD" >>"${LOG_FILE}"
    echo "$(date +%Y%m%d%H%M) : Success treating $PROD" >>"${ERR_FILE}"

    remove_work "${TEMP_OUT}"
    remove_work "${TEMP_WORK}"
    if [ -n "${RM_L1C}" ]; then
      echo "$(date +%Y%m%d%H%M) : Request removing input L1C $PROD"
      echo "$(date +%Y%m%d%H%M) : Request removing input L1C $PROD" >> "${LOG_FILE}"
      mv "$PROD" "$(dirname "$PROD")/.$(basename "$PROD")"
      rm -rf "$(dirname "$PROD")/.$(basename "$PROD")"
    fi
  ...
  done < "${WORK_DIR}/ListTLRandom.txt"
  echo "$(date +%Y%m%d%H%M) : Finished loop on products"
